local addonName, cxns = ... -- cxns = cxUI's shared addon namespace (deliberately unused below)

-- ===========================================================================
-- MODULE: SMALL TWEAKS -> NO AUTO CLOSE
--
-- Ported from the standalone "NoAutoClose" addon (by Numy, github.com/
-- NumyAddon/NoAutoClose). Stops Blizzard's UIPanel system from auto-closing
-- other panels whenever a new one is opened (e.g. opening the map no longer
...
local function IsEnabled()
    return CXUI_DB and CXUI_DB.noAutoClose
end
...
function ns:ConfigureSecureEscHandler(frame, alwaysSetBindingOnShow, callRegenDisabledCode)
    ...
    local name = 'CXUI_NoAutoClose_SecureEscapeHandlerFrame' .. self.handlerFrameIndex;
